...
int main() {
	...
	for (int nImage = 0; nImage<nImages; nImage++) {
		...
		for (int k=0; k<480*640; k++) if (disp[k]>2047) { cout << k << " " << disp[k] << endl; break; }
...
//		kinect.startImg(true, 2); //Start IR at high resolution
		for (int i=0; i<2; i++) //Discard first IR image
			ir = kinect.getIR();
		kinect.stopImg(); //Start IR at high resolution
			

		cout << "Calculate a Pattern" << endl;
		cv::Mat1d D = cv::Mat1d(cv::Mat1w(480, 640, &disp[0]))(cv::Rect(0,0,632,480));
		cv::Mat1d I = cv::Mat1d(cv::Mat1w(1024, 1280, &ir[0]))(cv::Rect(0,24,1280,976))/1024.;
